...
%ESTIMATE_POSE 6DOF vision based 3-D pose estimator which estimates position and
% orientation of the quadrotor based on AprilTag
...
K = varargin{2}; 
%%
if ( isempty(sensor.id)) % No april tags observed in  image 
    pos = []; 
    eul = []; 
else
% First construct A1 - A5,  5 points with known world coordinates 
...
%% For loop 
idx = sensor.id+1;  
x_p = [sensor.p0(1,:)', sensor.p1(1,:)', sensor.p2(1,:)',  sensor.p3(1,:)', sensor.p4(1,:)']; % x position of all the observed april tags
y_p = [sensor.p0(2,:)', sensor.p1(2,:)', sensor.p2(2,:)',  sensor.p3(2,:)', sensor.p4(2,:)']; % y position of all observed april tags 
A = []; 
...
%% Homography 
 ...
 %since we cannot guarentee that  h1 and h2 will give us orthagonal column vectors take SVD
